...
//
// claude -p is non-interactive: tools either auto-approve via --allowedTools
// or get denied. There's no interactive confirmation channel.
//
// - safe/standard: explicit allowedTools list
// - full: broad allowedTools including MCPs (uses tool whitelist, not bypass)
// - yolo: --dangerously-skip-permissions (bypasses ALL security checks)
//
const FULL_TOOLS = [
  ...
  'mcp__*',   // All MCP tools (wildcard prefix match)
].join(',')

export const PERMISSION_MODES = {
  safe:     { label: 'Safe',     desc: 'Read-only (Read, Glob, Grep)',              allowedTools: 'Read,Glob,Grep' },
  standard: { label: 'Standard', desc: 'Read + Write + Edit + Bash + search',       allowedTools: 'Read,Write,Edit,Bash,Glob,Grep,WebSearch,WebFetch' },
  full:     { label: 'Full',     desc: 'All tools + MCPs (auto-approved)',           allowedTools: FULL_TOOLS },
  yolo:     { label: 'YOLO',     desc: 'Bypass ALL security checks (dangerous)',     dangerous: true },
}
